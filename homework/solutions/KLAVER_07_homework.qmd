...
# check relevant priors
get_prior(mdl_formula, quantese)

# plot log rt distribution to get an idea of reasonable values
quantese |>
  ggplot(aes(x = log_rt)) +
  geom_density() +
  theme_minimal()
...
my_priors <- c(prior(normal(7, 3), class  = 'Intercept'),
...
# check whether chains are mixed
mcmc_plot(quant_mdl, type = 'trace')
  # chains appear to be reasonably mixed, some potentially problematic peaking

# check effective sample size
quant_mdl
  # bulk ESS < 400 for two parameters AND Rhat = 1.01 for three parameters, needs to be addressed

# again with longer warmup period
quant_mdl_2 <- brm(mdl_formula,
                 family = gaussian(),
                 prior = my_priors,
                 data = quantese,
                 # MCMC specs:
...
# check mcmc diagnostics again
mcmc_plot(quant_mdl_2, type = 'trace')
quant_mdl_2
  # problems solved!!!
```
...
post_samples_quant <- as_draws_df(quant_mdl_2,
                                  variable = c("Intercept", "b_log_frequency"))

# calculate means 
mean_post_samples <- summarise(post_samples_quant,
                               mean_int = mean(Intercept),
                               mean_slope = mean(b_log_frequency))

# calculate 95% Credible intervals using the Equal Tailed Interval method
post_ci <- ci(post_samples_quant,
         method = "ETI") |>
  select(-CI) |>
  pivot_wider(names_from = Parameter,
              values_from = c(CI_low, CI_high))

# plot predicted mean linear model on top of data
ggplot(quantese,
       aes(x = log_frequency, 
           y = log_rt)) +
  # plot raw data
  geom_point(alpha = 0.5,
             colour = "turquoise") +
  # plot mean linear model
  geom_abline(data = mean_post_samples,
              aes(slope = mean_slope, 
                  intercept = mean_int),
              linewidth = 1) +
  # plot CI as lines
  geom_abline(data = post_ci,
              aes(slope = CI_low_b_log_frequency,
                  intercept = CI_low_Intercept),
              linetype = "dashed") +
  geom_abline(data = post_ci,
              aes(slope = CI_high_b_log_frequency,
                  intercept = CI_high_Intercept),
              linetype = "dashed") +
  theme_minimal()
